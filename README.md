...
What *is* verified: the gates' and guards' 54 behaviours, tested deterministically with no model in the loop ([`scripts/test-gates.sh`](./scripts/test-gates.sh)); Antigravity's `Stop` hook genuinely blocking, run rather than read from documentation ([`docs/verified.md`](./docs/verified.md)); both plugin manifests, the rulebook hashes, and the leakage guard, all in CI; every skill executed at least once — the inception chain and `init` against this repository or a scratch clone of a real project, and `spec`, `clarify`, `implement` and `worklog` on every issue since #17 — with what each run found recorded in [`docs/verified.md`](./docs/verified.md) and filed as issues; and a reviewer that has reviewed every pull request since #17 as a subagent, with its receipts under `.specs/`.
