...
1. **Require a tracker issue. No issue, no spec.** `.steering/tech.md` names this project's tracker and the command to read issues. Read the issue you were given. If there is no issue, **stop and say so.** Do not create one to proceed. An issue created here is unplanned work entering through the side door, bypassing the sprint that was supposed to decide what gets built — and the sprint stops meaning anything the first time it is bypassed silently. Offer to run `sprint`, or to create the issue explicitly as acknowledged unplanned work; either way it is the user's call, made out loud. **Read `- Mode:` from `.steering/tech.md` before deciding which of those this is.** The accusation above is scoped to `full`, where `sprint` decides what gets built and an issue created here really does bypass it. In a `minimum`-mode project the backlog is the queue and creating the issue from it is the normal path — still explicit, still out loud, but not a side door. A project still in `bootstrap` has neither, so read it as `minimum` here — and tell the user that the document-set check expires `bootstrap` the moment this spec directory exists, because it is about to. A rule that fires on every single spec is one the user learns to skip, and it would take the rule that matters — no issue, no spec — with it. If the line is absent — every project installed before it existed — treat the project as `full` and say so out loud, rather than picking a mode silently. The checker refuses to default this value; prose that reads the same line should not default it either. The slug is `<n>-<kebab-title>`. It names **both** the branch and the spec directory, so an issue, a branch, a spec, and a PR are always the same unit of work. **The issue's type decides the spec's shape** — take it from the template used or the label applied: `feature`, `bug`, or `chore`. If it is genuinely unclear, ask. Do not default to feature; feature is the shape that fits a bug worst.
