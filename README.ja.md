...
**v0.4.3、pre-release です。** 検証済みバージョンの一覧を添えた reference implementation であって、サポート付きのプロダクトではありません。[eval スイート](./evals/) は開発中です。4つのケースは書いてありますが、`claude plugin eval` はこのアカウントではまだ early access のままです。2026-09-06 の時点でもヘルプは表示されるものの、実際に呼び出すと「in early access」と出力して終了し、ケースは1つも実行されません。そのため、まだ一度も走っていません。走らせたことのないスイートを「グリーンです」と言うのは、このハーネスが防ごうとしている未検証の主張そのものです。
...
**検証できていないこと。** skill が、答えをまだ持っていない人から判断を引き出せるかどうかです。これまでの実行で作者に投げた質問は、すべて推奨案どおりに答えられました。skill が動くことは示せましたが、それ以上のことは示せていません。eval スイートは開発中で、未検証です。`init` については、まだ何も導入していないリポジトリへの導入と、既存インストールのアップグレードという2つの経路を試していません。検証環境の Antigravity の行も、2026-08-21 以降は確認していません。
