...
**強制の強さは、モードによって一切変わりません。** ゲートも reviewer も receipt も TDD ループも同じです。違うのは、最初の spec を書くまでにどれだけ計画を書くか、それだけです。そしてこれは一方通行でもありません。`minimum` のプロジェクトでも、レビュー指摘が繰り返され始めた時点で `contract` を走らせればよく、すでにモードを宣言したプロジェクトに `init` を再実行すれば、入れ直しではなくアップグレードを提案します。モードは上にしか動きません。
