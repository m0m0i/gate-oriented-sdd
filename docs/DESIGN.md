...
| Receipt | the Receipt block's fields | reviewer → `implement` copies verbatim → `gate_spec_review_state` in `gate-lib.sh` reads, for `review-gate.sh` and `check-unreviewed-work.sh` alike | the gate blocks on a missing, non-CLEAN or stale `reviewed_sha`; an absent `reviewed_by` means not independent | `check-receipt-schema.py` keeps the copies of the schema agreeing |
| Unreviewed work | "this spec holds finished work with no usable review", as a state string | `gate_spec_review_state` → the turn-end gate (current branch, from the working tree or, failing that, from that branch's own tree, then every other unmerged local branch) and the pull-request check | the hook and CI would otherwise answer the same question differently; the hook alone answered it only about the branch HEAD pointed at (#26), and then only about the branch HEAD pointed at whose spec was checked out, because the current-branch reader consulted one tree and the scan skipped that branch by name (#177) | `test-gates.sh` — the scan's block, each of its silence cases, and one case per state the scan can reach — the two unreadable states cannot arise from a ref read in the ordinary way, since a blob a tree names is always readable — reaching one means the object store is damaged, which is why the turn-end gate's fallback corrects the permission remedy in those two messages (#177); the callers share one definition rather than agreeing by hand. Named rather than numbered, per G-9: the suite prints no case numbers, and this row was stale two commits after it was written |
