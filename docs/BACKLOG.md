...
| 2 | **#26** — the gate is silenced by `git checkout` | #25 | ~1–2 issues | The only enforced rule is one command from off, and silently: a skipped review and a clean repo produce identical output. It goes second and not first, though the original reason for that has since been retracted — see the note below the table. It holds now because #26 is the larger change and a bypass is safer to close once #27's wait instruction has bedded in. Fixing them in this order is the whole reason to write an order down. |
...
- **FIRED, 2026-08-25, and the order held.** #27's spec conceded that a blocking `Stop` hook
  ...
  (`docs/verified.md`). #26 stays second, but **not** on its original
  reasoning. "Today it is the only escape from #27" does not survive: a stuck turn has an exit
  that is not `git checkout`, and #27 ships the instruction that teaches it. `git checkout` is the
  only *bypass*, never the only *exit* — and that rationale would have expired on #27's merge in
  any case. The order holds on a different reason: #26 is the larger change, and closing a bypass
  is safer once the instruction removing the need for it has bedded in.
