...
- [ ] T1: failing test in `scripts/test-gates.sh` asserting `scripts/package-release.py` packages runtime files, preserves symlinks, and strictly excludes internal dev assets — then implement `scripts/package-release.py` to make the test pass
