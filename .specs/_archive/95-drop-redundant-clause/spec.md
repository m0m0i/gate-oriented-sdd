...
- Acceptance criteria:
  - [x] **AC1:** `grep -c "none has run yet" README.md` and `grep -c "まだ一度も走っていません" README.ja.md` are both 0; "exits without running a case" and 「ケースは1つも実行されません」 remain.
  - [x] **AC2:** `git diff --numstat main` shows 1/1 for each README and nothing else outside this directory; validators at exit 0 after the last write.
...
- [x] T2: remove; assert AC1–AC2, validators last.
