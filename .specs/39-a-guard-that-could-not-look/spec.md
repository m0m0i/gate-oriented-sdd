...
- [ ] T1: case that `quality-gate.sh` exits 0 with `{}` on a present-but-unreadable `.steering/tech.md` — then `gate_steering_read` in `hooks/gate-lib.sh`, `gate_steering_value` reimplemented over it, and `quality-gate.sh` opting in. (AC1, AC2, AC8)
