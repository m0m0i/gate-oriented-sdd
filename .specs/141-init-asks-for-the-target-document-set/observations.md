...
- **INFO (partly taken)** — `docs/DESIGN.md`, `CONTRIBUTING.md` and `scripts/test-gates.sh:485` all said "five machine-read lines"; there are now seven. Already stale at #110 and widened by this diff. `docs/DESIGN.md` and `CONTRIBUTING.md` are corrected here, along with the two READMEs documenting `- Target:`, because shipping a release whose own documentation is known-false is worse than the scope creep. **`scripts/test-gates.sh:485` is deliberately left**: it is inside `- Source globs:`, so editing a comment there re-stales the review receipt and buys a sixth review round for a sentence no gate reads. Worth its own issue, with the `- Mode:` unpinned-destination gap.
