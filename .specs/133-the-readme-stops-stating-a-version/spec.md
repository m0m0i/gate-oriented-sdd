...
- [x] T4: failing case — a `**v<x.y.z>` literal reappearing in either README's prose fails, saying that the badge is the source and a second one is the bug returning — then invert the existing `VERSION` pattern into a prohibition (AC5)
