...
- [ ] T1: failing case — a README with no dynamic version badge fails naming the file, and one carrying it passes — then swap the literal check for the badge check in `check-readme-claims.py` and put the badge row in both READMEs with the number removed from `## Status` (AC1, AC2)
