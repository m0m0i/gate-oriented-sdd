...
- Slug: 131-reviewer-allow-list-drifts-from-the-validators-line   Issue: 131   Type: bug   Status: draft
- Author: m0m0i   Date: 2026-09-14
...
**No shipped file changes.** `check-version-bump.py`'s `SHIPPED` is `skills/ agents/ hooks/ assets/
plugin.json`; every path above is outside it, and `.claude/agents/` is the dogfood install rather
than `agents/`. So this branch takes **no version bump** — which is also why it cannot repeat the
#136 hazard the last two branches hit.
...
- [ ] **T4:** AC9 — extend the `python3 -O` case to both new branches (#28), and confirm AC6 by
      diffing every reviewer's Bash policy for added commands.
