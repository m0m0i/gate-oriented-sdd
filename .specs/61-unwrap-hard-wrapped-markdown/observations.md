...
Fixed with a rule rather than an edit, as the slot case was — and the first version of the rule was too broad, which the next review caught.

**The rule, as it stands.** A continuation sitting at an **enclosing** open item's content column belongs to that outer item, so the transform closes the item instead of joining. Deliberately "equals an enclosing column" rather than "less than the current one": a continuation at column 0 under an indented item is usually a plain hard wrap that *should* join — `.specs/55-run-northstar-prd-epics/spec.md` puts one there so a code span does not gain a stray space — and the broader test refused it. That left `unwrap.py` unable to reproduce its own output, which is the property AC1 and AC2 rest on.

**The survey, with its predicate stated so it can be re-run.** Scanning `main` for continuations whose indent equals an enclosing open item's content column, fence- and slot-aware, gives exactly **one** firing: `skills/sprint/SKILL.md:28`. The other two cases are closed by different rules and were wrongly attributed to this one in an earlier version of this note — `agents/_template/rules/starter.md`'s `-->` by treating HTML comment delimiters as structure, and the two template slots by `SLOT`. An earlier count of "five instances" was not reproducible under any predicate; it is one, plus one, plus two, by three separate rules.
