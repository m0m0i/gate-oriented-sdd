...
- Acceptance criteria:
  - [x] **AC1:** WHEN a reviewer shipped in `agents/` is copied into an installed project THE SYSTEM SHALL name the reviewer contract by a path that resolves there, under both harness layouts.
  - [x] **AC2:** The three reference reviewers, `agents/_template/reviewer.md`, `skills/init/SKILL.md`, `docs/layout.md` and `scripts/check-receipt-schema.py` all state the same placement; `init` names the exact path rather than "next to it".
  - [x] **AC3:** WHEN any of those files states a different placement from the others THE SYSTEM SHALL fail a validator naming the disagreeing file. The defect is that one fact is written in five places and nothing compares them — a fix that leaves the comparison to review re-opens it on the next edit.
  - [x] **AC4:** WHEN a reviewer's stated contract path does not exist THE SYSTEM SHALL still behave as `agents/_shared/reviewer-contract.md:5` prescribes — say so and stop. This is not relaxed; AC1 removes the cause rather than the symptom.
  - [x] **AC5:** `.claude/agents/` — this repository's own install — continues to resolve, and `scripts/check-receipt-schema.py`'s mirror pairing keeps working against whatever placement is chosen.
  - [x] **AC6:** the regression tests fail before the fix and pass after, in both directions: a reviewer whose path resolves, and one whose path does not.
  - [x] **AC7:** THE SYSTEM SHALL carry a version bump in both manifests — `agents/` and `skills/` are shipped paths — as the step of `implement` after the receipt, not as a task (#113). **The size is `0.7.0`, a minor** — _decided 2026-09-12 after the receipt, on the reviewer's verdict across rounds 3-6._ `1.0.0` was reserved for the minimum/full switch **and** a settled installed interface; #110 delivered the first and this issue was to be the second. It is not. This branch was wrong **twice** about where a reviewer can open its contract — plugin-relative, then reviewer-relative — and both times the error was invisible to a full green validator set and obvious within seconds to something that tried to use the instruction. Six rounds made the *guards* trustworthy, which is a different claim from the *interface* being settled. `1.0.0` waits for an `init` run against a scratch clone showing a shipped reviewer opening its contract with no hand-edit, recorded as a `docs/verified.md` row — a run already owed for #81, #83 and #84, so the evidence is one scheduled step away rather than a new commitment.
...
- 2026-09-12 — **Absolute or relative?** The issue says *"Pick one: `.claude/agents/_shared/reviewer-contract.md`"*, which is Claude Code's directory, while these reviewers ship to Antigravity too (`.agents/`, `docs/layout.md:49`). **Answered: relative to the reviewer — `_shared/reviewer-contract.md`.** One string is then correct under both layouts, and `agents/_template/reviewer.md` already says exactly that, making it the only file in the repository that is right today. The issue's own proposal is therefore **overruled**, and the reason is recorded rather than left to a reader to rediscover: an absolute path in a file that ships to two harnesses is wrong for one of them by construction, and `docs/fidelity.md:8` already calls that contract "partial — untested" rather than broken. Documents still draw the concrete tree, because a diagram cannot draw a relative path. Rejected: dropping `_shared/` for a flat sibling, which matches what `docs/layout.md` draws today but would move this repository's own file and rewrite `check-receipt-schema.py`'s mirror pairs for no gain. Serving AC1, AC2. _**Amended 2026-09-12, after review round 1.** The answer above is half right and was shipped as if it were whole. The relative form alone resolves from **neither** project root: a reviewer reads its own instruction with the project root as its working directory, not the directory its own file sits in — found by a reviewer following the new line literally, failing, and guessing the plugin's copy. The answer is the relative form **plus** the concrete destination for each harness. Recorded here rather than only in `observations.md`, because this clarification is what a later spec reads when it wants to know how this was settled, and leaving it uncorrected is how the reasoning error reaches the next file that ships to two harnesses._
