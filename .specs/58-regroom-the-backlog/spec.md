...
- Acceptance criteria:
  - [ ] **AC1:** WHEN the grooming completes, the sorted set of issue numbers in the table's Item column equals the sorted set from `gh issue list --state open`, each appearing exactly once, demonstrated by a recorded diff of the two lists; and every row carrying no issue number says so in its Item cell, with those rows listed in `observations.md` for `sprint` to take from.
