...
`reviewed_by` is the one field you may not know the answer to from inside your own run: say `subagent` if you were spawned as an agent, and `inline` if your procedure is being followed by the author of the diff. An **absent** `reviewed_by` means unknown — receipts written before this field existed must not be read as independent, because silence is not evidence.
